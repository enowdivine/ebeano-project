...
<div class="row">
    ...
    <div class="row">
        <div class="col-md-12">
            <a class="btn btn-tsk  mb-2" href="{{route('room_type')}}"><i class="fa fa-list"></i> Room type List</a>
        </div>
    </div>
    <div class="row">
        <div class="col-md-8">
            <div class="card">
                <div class="card-body">
                    ...
                    <dl class="row">
                        ...
                        <dd class="col-md-6">{{number_format($roomType->base_price,2)}} &#8358;</dd>
                        <dt class="col-md-6">Additional Person Price :</dt>
                        <dd class="col-md-6">{{number_format($roomType->additional_person_price,2)}} &#8358;</dd>
                        <dt class="col-md-6">Extra Bed Price :</dt>
                        <dd class="col-md-6">{{number_format($roomType->extra_bed_price,2)}} &#8358;</dd>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if($roomType->getDayByRegularPrice($key)['amount_type'] ==='ADD')
                                            {{number_format($roomType->getDayByRegularPrice($key)['amount'],2)}} &#8358;
                                            @elseif($roomType->getDayByRegularPrice($key)['amount_type'] ==='LESS')
                                        <span class="text-danger">(   {{number_format($roomType->getDayByRegularPrice($key)['amount'],2)}} &#8358; )</span>
                                        @endif
                                    </td>
                                    @endforeach
                                </tr>
                                ...
                                <tr>
                                    ...
                                    <td class="text-center">
                                        @if($special_price->getDayByRegularPrice($key)['amount_type'] ==='ADD')
                                            {{number_format($special_price->getDayByRegularPrice($key)['amount'],2)}} &#8358;
                                            @elseif($special_price->getDayByRegularPrice($key)['amount_type'] ==='LESS')
                                        <span class="text-danger">(   {{number_format($special_price->getDayByRegularPrice($key)['amount'],2)}} &#8358; )</span>
                                        @endif
                                    </td>
                                    @endforeach
                                </tr>
        ...
        <div class="col-md-4">
            <div class="card">
                <div class="card-body">
                    <h6><strong>Image</strong> <form action="{{route('room_type_upload_image')}}" class="float-right" method="post" enctype="multipart/form-data">@csrf
                                ...
                                <div class="img img-thumbnail">
                                    ...
                                    <div class="featured" >
                                        @if($image->featured)
                                            FEATURED
                                        @else
                                            <a href="{{route('room_type_set_as_featured',[$roomType->id,$image->id])}}" class="btn btn-sm btn-danger">Set as featured</a>
                                        @endif
                                    </div>

                                    <div class="delete-img-btn">
                                        <form action="{{route('room_type_delete_image')}}" method="post" id="delete_img_form_{{$image->id}}">
                                            @csrf
                                            <input type="hidden" name="room_type" value="{{$roomType->id}}">
