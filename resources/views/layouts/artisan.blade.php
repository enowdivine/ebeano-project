...
@endif
    ...
    <title>@yield('title', config('app.name', 'Ebeano Artisans'))</title>
...
<body>
    ...
    <div class="header">
        @include('inc/artisan_welcome_nav')
    </div>
...
<script>
    ...
    function showViewJobDetails(id){
        if(!$('#modal-size').hasClass('modal-lg')){
            $('#modal-size').addClass('modal-lg');
        }
        $('#viewJobDetails-modal-body').html(null);
        $('#viewJobDetails').modal();
        $('.c-preloader').show();
        $.post('{{ route('cart.showCartModal') }}', {_token:'{{ csrf_token() }}', id:id}, function(data){
            $('.c-preloader').hide();
            $('#viewJobDetails-modal-body').html(data);
    ...
    function bidNow(){
        if(checkBidValidity()) {
            ...
            $.ajax({
               ...
               data: $('#option-choice-form').serializeArray(),
               success: function(data){
                   //window.location.replace("{{ route('bids') }}");
               }
           });
        }
        else{
    ...
    function show_bid_history_details(bid_id)
    {
        $('#bid-details-modal-body').html(null);

        if(!$('#modal-size').hasClass('modal-lg')){
            $('#modal-size').addClass('modal-lg');
        }

        $.post('{{ route('purchase_history.details') }}', { _token : '{{ @csrf_token() }}', order_id : bid_id}, function(data){
            $('#bid-details-modal-body').html(data);
            $('#bid_details').modal();
            $('.c-preloader').hide();
        });
    }

    function show_bid_details(bid_id)
    {
        $('#bid-details-modal-body').html(null);
        ...
        $.post('{{ route('orders.details') }}', { _token : '{{ @csrf_token() }}', order_id : bid_id}, function(data){
            $('#bid-details-modal-body').html(data);
            $('#bid_details').modal();
            $('.c-preloader').hide();
        });
